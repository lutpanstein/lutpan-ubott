    ...
    def decorator(func):
        async def wrapper(event):
            ...
            if admins_only:
                if event.is_private:
                    return await edit_delete(
                        event, "**DIPAKENYA DIGRUB YA BANG.**", 10
                    )
                if not (chat.admin_rights or chat.creator):
                    return await edit_delete(
                        event, f"**LU GA NGETMIN DISINIIBKONTOOLLLL {chat.title}**", 10
                    )
            if group_only and not event.is_group:
                return await edit_delete(
                    event, "**DIPAKENYA DIGRUBBB DOANG KONTOOOLLL.**", 10
                )
            if private_only and not event.is_private:
                return await edit_delete(
                    event, "**PAKE DIPRIVATEEEE MEMEKK ANJINGG CUKIMAYY BABIII.**", 10
                )
            try:
                await func(event)
            # Credits: @mrismanaziz
            ...
            # t.me/SharingUserbot & t.me/Lunatic0de
                ...
                await edit_delete(
                    event, "**INLINENYA DINYALAINNYA KONTOOOLLL**", 10
                )
            except ChatSendStickersForbiddenError:
                await edit_delete(
                    event, "**GABISA NGIRIM STIKER DISINI COKK ETMIN SINI PELIT**", 10
                )
                ...
                await edit_delete(
                    event, "**ETMINNNN SINII PELITTT GABISA NGIRIM TIKELLL**", 10
                )
            except AlreadyInConversationError:
                await edit_delete(
                    event,
                    "**KATA LUTPAN GAKENA LIMIT AKUNLUU.. MOGA NTAR LIMIT PERMANEN.**",
                )
                ...
                LOGS.error(
                    f"FLOODWAIT {e.seconds} KONTOLLLL WKWKWKWKWK"
                )
                await event.delete()
