    ...
    for snip in snips:
        ...
        if re.search(pattern, name, flags=re.IGNORECASE):
            try:
                ...
                await event.reply(
                    "`KATA LUTPAN LU GA NGETMIN DISINI KONTOL AJG BABI MMK CUKIMAY NGENTOD`"
                )
                await sleep(1)
                await reply.delete()
...
async def on_add_black_list(addbl):
    ...
    await edit_or_reply(
        addbl, "`KONTOL DIKIT DIKIT BL` **{}** `DAH DONE YA AJG`".format(text)
    )
...
async def on_view_blacklist(listbl):
    ...
    if len(all_blacklisted) > 0:
        ...
        OUT_STR = "**NI KONTOLL BLACKLIST DISINII.**"
    if len(OUT_STR) > 4096:
...
async def on_delete_blacklist(rmbl):
    ...
    if not successful:
        ...
        await rmbl.edit("`DONE BANGGG UDAHHH GA KEHAPUS NI KATA² NYA **{}**`".format(text))
