...
NO_ADMIN = "**LU GA NGETMINN DISINI KONTOLLL MENDING LU NGETMIN DIGC LUTPAN**"
NO_PERM = "**BELI PREM DULUUU DEKKK KONTOL BABI CUKIMAYY MEMEKKKK AJGGGGGGGG**"
...
async def demote(event):
    ...
    try:
        ...
        return await eventman.edit(NO_PERM)
    await edit_delete(eventman, "`udah ga etmin bang makhluk yg lu un etmin`", 30)
...
async def ban(bon):
    ...
    if not user:
        return
    man = await edit_or_reply(bon, "`KATA LUTPAN LAGI GIBENN NI MAKHLUKKK YG LU GIBEEEEENNNN KONTOOOLLL..`")
    try:
    ...
    if reason:
        await man.edit(
            r"\\**BEJIRRRR DIBANEEEDDDDD**//"
            f"\n\n**NI MAKHLUK YG KENA BAN** [{user.first_name}](tg://user?id={user.id})\n"
            f"**TOGELNYA :** `{str(user.id)}`\n"
            f"**KATA MAKHLUK YG NGEBAN GARA² INI:** `{reason}`",
        )
    else:
        await man.edit(
            f"\\\\**BEJIRRRR DIBANEEDD**//\n\n**MAKHLUK YG DIBAN :** [{user.first_name}](tg://user?id={user.id})\n**TOGELNYA :** `{user.id}`\n**GARA² :** `YG NGEBANNED INII MAKHLUKNYAAA {me.first_name}`",
        )
...
async def nothanos(unbon):
    ...
    man = await edit_or_reply(unbon, "`LAGI NGLEPAS MAKHLUK YG LU BAN TDI BIARR LEPASS KAYA BURUNG YG TERBANG BEBASSS KATA LUTPAN...`")
    ...
    try:
        ...
        await edit_delete(man, "`DONE ANJING BABI MEMEK KONTOL CUKIMAY NGENTOD`")
    except UserIdInvalidError:
        await edit_delete(man, "`AWOKAWOAKKAOWKAKSKSJISKSJSK ERROR BANG`")
...
async def spider(spdr):
    ...
    man = await edit_or_reply(spdr, "`WAITT BANG LAGI NGEMUT AH AH AH AHHH...`")
    user, reason = await get_user_from_event(spdr)
    if not user:
        return
    self_user = await spdr.client.get_me()
    if user.id == self_user.id:
        return await edit_or_reply(man, "**LU MASA MAU NGEMUT DIRI SENDIRI BANG.. LU DIEM AJA BISA KALI... FAK KATA LUTPAN TEH**")
    if user.id in DEVS:
        return await man.edit("**NI DEVELOPER GANTENG LU EMUT WKWKWKWKWK**")
    if user.id in WHITELIST:
        return await man.edit("**Gagal Mute, dia adalah admin DI GC GW WKWKWKWKKWWKWK🤪**")
    await man.edit(
        r"\\**BEJIRRR DIEMUT GA TUHHH**//"
        f"\n\n**NI MAKHLUKNYA :** [{user.first_name}](tg://user?id={user.id})\n"
        f"**TOGELLL:** `{user.id}`\n"
        f"**SIAPA YG MAENN EMUTTT?? :** `NIIHH YG MAIN EMUT {self_user.first_name}`",
    )
    if mute(spdr.chat_id, user.id) is False:
        return await edit_delete(man, "**ERROR KONTOL** `UDAH DIEMUT MAU DIEMUT LAGI MAU DIEMUT BERAPA KALI LAGI ANJINGGGGG `")
    try:
        await spdr.client(EditBannedRequest(spdr.chat_id, user.id, MUTE_RIGHTS))
        if reason:
            await man.edit(
                r"\\**AWIKAOWKWKWOAKWKWK DIMUTE**//"
                f"\n\n**NI MAKHLUKNYA :** [{user.first_name}](tg://user?id={user.id})\n"
                f"**NI TOGELNYA :** `{user.id}`\n"
                f"**GARA² INI KATANYA** `{reason} `",
            )
        else:
            await man.edit(
                r"\\**AJGGG DIEMUTTT AWIKAWOKAKWKOK**//"
                f"\n\n**NI MAKHLUKNYA :** [{user.first_name}](tg://user?id={user.id})\n"
                f"**NI TOGELNYA :** `{user.id}`\n"
                f"**ATRAKSIII:** `DIEMUT OLEH {self_user.first_name}`",
            )
    except UserIdInvalidError:
        return await edit_delete(man, "**ERORRRR BABII KATA LUTPAN!**")
...
async def unmoot(unmot):
    ...
    man = await edit_or_reply(unmot, "`WAIITTT KATA LUTPANNN`")
    ...
    if unmute(unmot.chat_id, user.id) is False:
        return await edit_delete(unmot, "**UDAH GA DIEMUT KONTOL.**")
    try:
        await unmot.client(EditBannedRequest(unmot.chat_id, user.id, UNBAN_RIGHTS))
        await edit_delete(man, "**DAH GA DIEMUT!**")
    except UserIdInvalidError:
        return await edit_delete(man, "**ERROR BANG KATA LUTPAN GATAU KENAPA**")
...
async def ungmoot(un_gmute):
    ...
    man = await edit_or_reply(un_gmute, "`WAITTT YA PEPEEEKKKK...`")
    user = await get_user_from_event(un_gmute)
    user = user[0]
    if not user:
        return
    await man.edit("`LAGU UNGMUTEEE...`")
    if ungmute(user.id) is False:
        await man.edit("**ERROR!** GA GLOBAL EMUT BANG.. LU DOANG KALI YG NGEMUT DIGC LU.")
    else:
        await edit_delete(un_gmute, "**DONE KONTOOOLLL**")
...
async def gspider(gspdr):
    ...
    man = await edit_or_reply(gspdr, "`BEJIRR OKE BANGG WAITTR`")
    user, reason = await get_user_from_event(gspdr)
    if not user:
        return
    self_user = await gspdr.client.get_me()
    if user.id == self_user.id:
        return await man.edit("**GABISA KONTOLL..**")
    if user.id in DEVS:
        return await man.edit("**DEVELOPER KOK DIEMUT KOCAK**")
    if user.id in WHITELIST:
        return await man.edit("**GABISAAAAA WKWKWKEKWKEKKEKWKEK**")
    await man.edit("**DONEEEE KENAAA EMUT MASAAALLL YA KONTOL**")
    if gmute(user.id) is False:
        await edit_delete(gspdr, "**ERROR! UDAH DONE DARI DULU KONTOL .**")
    elif reason:
        await man.edit(
            r"\\**#GMuted_User**//"
...
async def get_admin(show):
    ...
    mentions = f"<b>☘ **NI BANG DAFTAR ETMIN DISINI KATA LUTPAN**{title}:</b> \n"
    try:
