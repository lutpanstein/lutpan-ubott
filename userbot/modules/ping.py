...
lutpan = [ 
     "**Bejirrr ada lutpan**",
     "**Kemana aja Baru Nongoll**",
     "**ROAARRR DULUUU**",
     "**HOAAAAMMMM Lutpan kok ganteng si**",
...
async def _(ping):
    ...
    start = datetime.now()
    xx = await edit_or_reply(ping, "**BENTAR KATA LUTPAN**")
    await xx.edit("**LAGI NGEPING**")
    await xx.edit("**GATAU PING LU SEGEDE APA**")
    await xx.edit("** NI PING LU SEGINI PEPEKK LIAT**")
    end = datetime.now()
...
async def _(ping):
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    xping = await edit_or_reply(ping, "`Lagi ngeping kontollll....`")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await xping.edit(
...
async def risman(ganteng):
    ...
    @register(pattern=r"^\.lutpan$", sudo=True)
async def lutpan(ganteng):
    await ganteng.reply(choice(absen))
