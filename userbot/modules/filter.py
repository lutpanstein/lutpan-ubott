...
async def add_new_filter(event):
    ...
    success = "**DONE MASUKUN {} BUAT FILTER**TERUS INI BALESAN OTOMATISNYA `{}` "
...
async def on_snip_list(event):
    OUT_STR = "**GADA KATA FILTER DISINI KONTOLLL.**"
    filters = get_filters(event.chat_id)
    for filt in filters:
        if OUT_STR == "**UDAH GADA FILTER DISINI PEPEK.**":
            OUT_STR = "**NI DAFTAR FILTER DISINI :**\n"
        OUT_STR += "• `{}`\n".format(filt.keyword)
    await edit_or_reply(
        event,
        OUT_STR,
        caption="KALO BISA JANGAN DIHAPUS BIAR LU KAYA BOT ROSE GACORNYA KATA LUTPAN",
