...
async def gcast(event):
    if xx := event.pattern_match.group(1):
        ...
        return await edit_delete(event, "**KATA LUTPAN DIREPLY BIAR DONE BANGGGG**")
    kk = await edit_or_reply(event, "`LAGI DERESIN GIKESANLU MESKIPUN GIKESANLU GA SEDERES GIKESAN LUTPAN...`")
    er = 0
    done = 0
        ...
        if x.is_group:
            ...
            if chat not in GCAST_BLACKLIST and chat not in BLACKLIST_GCAST:
                try:
                    ...
                    er += 1
    await kk.edit(
        f"**DAH KEKIRIM KE ** `{done}` ** GC BANG ** SEGINI NIH YG GA KEKIRIM LU KENA MUTE KALI WKWKWKWKWKWK**`{er}` "
    )
...
async def add(event):
    ...
    await xxnx.edit(
        f"**`{gc}` GC INI UDAH GA KENA GIKES YA ANJINGGGG.**\n\n**BEDEWE KALO KATA LUTPAN PELIT BET ETMINNYA KLO NYURUH ADDBL WKWKWKWKWKWK**."
    )
...
async def _(event):
    xxx = await edit_or_reply(event, "`waittt kontooollll`")
    gc = event.chat_id
    if HEROKU_APP_NAME is not None:
        app = Heroku.app(HEROKU_APP_NAME)
    else:
